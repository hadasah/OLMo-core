...
def main(
    ...
    **kwargs,
    ...
    if relaunch_path or relaunch_name:
        ...
        if relaunch_name:
            relaunch_path = os.path.join(
                PROJECT_SPECS[user]["DEFAULT_SAVE_PATH"], relaunch_name
            )
        ...
        for model in MODELS:
            model_sweep_name = (
                f"{SWEEP_NAME}_{model}" if add_model_to_name == "end" else SWEEP_NAME
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--add-time-to-name", type=str, default="front", choices=["front", "none"]
    )
