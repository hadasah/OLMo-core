...
class TransformerConfig(ModelConfig):
    ...
    @classmethod
    def olmo2_ml_1B(cls, vocab_size: int, **kwargs) -> "TransformerConfig":
        # Ported 2026-08-09 from atindra_exps commit 5fa9b55 -- the exact commit the
        # klone 1B sweeps (2026_07_31-01_20_02 moe64, 2026_07_31-23_36_15 dense) were
        # launched from, per each wandb run's metadata. Two kwargs from that version
        # (eom_prob, fom_prob) are dropped because this branch's MoERouterConfig /
        # MoEConfig do not have those fields; the klone 1B runs passed None for both,
        # so the built config is identical. Verified against the klone run configs:
        # d_model=1664, n_layers=15, n_heads=16, expert hidden 0.25*4*1664=1664.
        d_model = kwargs.pop("d_model", 1664)
        use_moe = kwargs.pop("use_moe", False)
        model_name = TransformerType.moe if use_moe else TransformerType.default
        generalist_hidden_multiplier = (
            kwargs.pop("moe_generalist_hidden_multiplier", 1) if use_moe else 1
        ...
        return cls.llama_like(
            ...
            vocab_size=vocab_size,
            hidden_size_multiplier=1.5,
            n_layers=kwargs.pop("n_layers", 15),
            n_heads=kwargs.pop("n_heads", 16),
            name=kwargs.pop("name", model_name),
            block_name=kwargs.pop("block_name", default_block_name),
            qk_norm=kwargs.pop("qk_norm", True),
